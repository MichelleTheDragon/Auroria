...
namespace Auroria
{
    public class WorldAssembler
    {
        ...
        public void LoadContent(ContentManager content, GameWorld gameWorld, GraphicsDeviceManager graphics)
        {
            ...
            gameWorld.OccupiedTiles = new GameObject[mapWidth, mapHeight];
            ...
            for (int y = 0; y < mapHeight; y++)
            {
                for (int x = 0; x < mapWidth; x++)
                {
                    ...
                    {
                        Rock myRock = new Rock(tileSet, rock, new Vector2(x * 64, y * 64));
                        gameWorld.AddObject(myRock);
                        gameWorld.OccupiedTiles[x, y] = myRock;
                    }
                    else if (colors[y * mapWidth + x] == green) //if the pixel is green, place a tree object
                    {
                        Tree myTree = new Tree(tileSet, tree, new Vector2(x * 64, y * 64));
                        gameWorld.AddObject(myTree);
                        gameWorld.OccupiedTiles[x, y] = myTree;
                    }
